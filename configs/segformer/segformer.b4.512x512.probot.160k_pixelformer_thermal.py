...
model = dict(
    ...
    auxiliary_head=dict(
        ...
        dict(type='SupervisedDepthLoss', num_scales=1, loss_type='Sparse-silog',loss_weight=1.0)
        ]),
...
val_depth_evaluator = dict(type='DepthMetrics', min_depth=0.1, max_depth=max_depth, depth_metrics=['RMSE'])
test_evaluator = val_evaluator

# The `val_interval` is the original `evaluation.interval`.
train_cfg = dict(type='TwoDataloadersIterBasedTrainLoop', DepthDataloader=train_depth_dataloader, max_iters=320000, val_interval=4000)
# train_cfg = dict(type='IterBasedTrainLoop', dataloader=train_depth_dataloader, max_iters=160000, val_interval=2000)
val_cfg = dict(type='TwoDataloadersValLoop', DepthDataloader= val_depth_dataloader, DepthEvaluator=val_depth_evaluator) # Use the default validation loop.
...
work_dir = '/home/iariav/Deep/Pytorch/mmsegmentation/work_dirs/b4_sgd_pixelformer_thermal_65m'