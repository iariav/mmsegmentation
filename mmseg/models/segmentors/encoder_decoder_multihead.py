...
class EncoderDecoderMultihead(BaseSegmentor):
    ...
    def __init__(self,
        ...
        self.with_segmentation_head = True
    ...
    def postprocess_depth_result(self, pred_depth, data_samples):
        """ Convert results list to `SegDataSample`.
                ...
                """
        batch_size, C, H, W = pred_depth.shape
        ...
        for i in range(batch_size):
            if not only_prediction:
                ...
                i_pred_depth = pred_depth[i]
                flip = img_meta.get('flip', None)
                if flip:
                    flip_direction = img_meta.get('flip_direction', None)
                    assert flip_direction in ['horizontal', 'vertical']
                    if flip_direction == 'horizontal':
                        i_pred_depth = i_pred_depth.flip(dims=(3,))
                    else:
                        i_pred_depth = i_pred_depth.flip(dims=(2,))

                # resize as original shape
                # print(i_pred_depth.shape)
                # print(img_meta['ori_shape'])
                # i_pred_depth = resize(
                #     i_pred_depth,
            ...
            data_samples[i].set_data({
                'pred_depth':
                    PixelData(**{'data': i_pred_depth.squeeze(0)})
            })
