...
class SupervisedDepthLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_weight=1.0,
                 loss_type='L1',
                 loss_name='loss_supervised_depth'):
        super().__init__()
        self.num_scales = num_scales
        self.loss_type = loss_type
        self.loss_weight = loss_weight

        if self.loss_type.endswith('L1'):
            self.criterion = nn.L1Loss()
        elif self.loss_type.endswith('mse'):
            self.criterion = nn.MSELoss()
        elif self.loss_type.endswith('berhu'):
            self.criterion = BerHuLoss()
        elif self.loss_type.endswith('silog'):
            self.criterion = SilogLoss()
        elif self.loss_type.endswith('abs_rel'):
            self.criterion = lambda x, y: torch.mean(torch.abs(x - y) / x)
        else:
            raise ValueError('Unknown supervised loss {}'.format(self.loss_type))

        self._loss_name = loss_name

    def calculate_loss(self, inv_depth, gt_inv_depth):
        """
    ...
    def forward(self,
        ...
        loss = self.loss_weight * self.calculate_loss(
            inv_depth,
            gt_inv_depth)
